import { Gift } from 'lucide-react';
import React from 'react';

const ChapterTopicList = ({ course }) => {
    const courseLayout = course?.courseJson?.course;

    return (
        <div className="flex items-center justify-center min-h-screen">
            <div className='mt-5 w-full max-w-2xl'>
                <h2 className='text-white font-bold text-2xl text-center'>Chapters & Topics</h2>
                <div className='flex flex-col items-center justify-center mt-10'>
                    {courseLayout?.chapters.map((chapter, index) => (
                        <div key={index} className='flex flex-col items-center'>
                            <div className='p-4 border shadow rounded-xl bg-primary text-white'>
                                <h2 className='text-center '>Chapter {index + 1}</h2>
                                <h2 className='font-bold text-lg text-center'>{chapter.chapterName}</h2>
                                <h2 className='text-xs flex justify-between gap-16'>
                                    <span>Duration: {chapter?.duration}</span>
                                    <span>No. Of Chapters: {chapter?.topics?.length}</span>
                                </h2>
                            </div>
                            <div>
                                {chapter?.topics.map((topic, idx) => (
                                    <div className='flex flex-col items-center' key={idx}>
                                        <div className='h-10 bg-gray-300 w-1'></div>
                                        <div className='flex items-center gap-5'>
                                            <span className={`${idx % 2 === 0 && 'text-transparent'} ${idx % 2 !== 0 && 'text-white'} max-w-xs `}>{topic}</span>
                                            <h2 className='text-center rounded-full bg-gray-300 px-6  p-4'>{idx + 1}</h2>
                                            <span className={`${idx % 2 !== 0 && 'text-transparent'} ${idx % 2 === 0 && 'text-white'} max-w-xs`}>{topic}</span>
                                        </div>
                                        {idx === chapter?.topics?.length - 1 && <div className='h-10 bg-gray-300 w-1'></div>}
                                        {idx === chapter?.topics?.length - 1 && (
                                            <div className='flex items-center gap-5'>
                                                <Gift className='text-center rounded-full bg-gray-300 h-14 w-14  p-4' />
                                            </div>
                                        )}
                                        {idx === chapter?.topics?.length - 1 && <div className='h-10 bg-gray-300 w-1'></div>}
                                    </div>
                                ))}
                            </div>
                        </div>
                    ))}
                    <div className='p-4 border shadow rounded-xl bg-green-600 text-white'>
                        <h2>Finish</h2>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default ChapterTopicList;
